...
HWU<-function(Tn,K,geno,X=NULL,center.geno=F,gsim=c("add","eq","dist"),appx=c("davis","normal"))
{
  appx<-match.arg(appx);
  gsim<-match.arg(gsim);
...
NHWU<-function(Tn,geno,X=NULL,center.geno=F,gsim=c("add","eq"),appx=c("davis","normal"))
{
  appx<-match.arg(appx);
  gsim<-match.arg(gsim);
  K<-matrix(1,nrow(geno),nrow(geno));
  HWU.core(Tn,K,geno,X,center.geno,appx=appx);
}
...
weight.gaussian<-function(x,weight=NULL)
{
...
weight.cov<-function(x,weight=NULL)
{
  x<-apply(x,2,map.std.norm);
  if(is.null(weight)) weight<-rep(1,ncol(x));
